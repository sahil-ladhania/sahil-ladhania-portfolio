"use client";
/**
 * Source: https://ui.aceternity.com/components/terminal
 * Registry: npx shadcn@latest add @aceternity/terminal
 */
  ...
  useEffect(() => {
    ...
    const init = async (): Promise<void> => {
      try {
        ...
        // Sound file unavailable — silent fallback
      }
    };
    void init();
    return () => {
...
const tokenColors: Record<TokenType, string> = {
  ...
  default: "text-foreground-muted dark:text-neutral-300",
};

function SyntaxHighlightedText({ text }: { text: string }): React.ReactElement {
...
export function Terminal({
  ...
  const soundEnabled = enableSound && !prefersReducedMotion;
  const { down, up } = useAudio(soundEnabled);

  const [lines, setLines] = useState<TerminalLine[]>([]);
  const [currentText, setCurrentText] = useState<string>("");
  const [commandIdx, setCommandIdx] = useState<number>(0);
  const [charIdx, setCharIdx] = useState<number>(0);
  const [outputIdx, setOutputIdx] = useState<number>(-1);
  const [phase, setPhase] = useState<
    "idle" | "typing" | "executing" | "outputting" | "pausing" | "done"
  >("idle");
  const [cursorVisible, setCursorVisible] = useState<boolean>(true);
  const [instantLoaded, setInstantLoaded] = useState<boolean>(false);
  ...
  useEffect(() => {
    if (!inView || instantLoaded || !prefersReducedMotion) return;

    const instantLines: TerminalLine[] = [];
    commands.forEach((command, index) => {
      instantLines.push({ type: "command", content: command });
      (outputs[index] ?? []).forEach((line) => {
        instantLines.push({ type: "output", content: line });
      });
    });
    setLines(instantLines);
    setPhase("done");
    setInstantLoaded(true);
  }, [inView, prefersReducedMotion, instantLoaded, commands, outputs]);

  useEffect(() => {
    if (!inView || phase !== "idle" || prefersReducedMotion) return;
    const t = setTimeout(() => setPhase("typing"), initialDelay);
    return () => clearTimeout(t);
  }, [inView, phase, initialDelay, prefersReducedMotion]);
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(t);
  }, [phase, charIdx, currentCommand, typingSpeed, down, up, prefersReducedMotion]);
  ...
  return (
    <div
      ref={containerRef}
      className={cn("w-full font-mono text-xs sm:text-sm", className)}
    >
      <div className="overflow-hidden rounded-lg border border-border-strong bg-background-subtle shadow-[0_8px_32px_rgba(8,74,58,0.12)] ring-1 ring-white/50 dark:bg-[#111a1d] dark:shadow-[0_8px_32px_rgba(0,0,0,0.45)] dark:ring-white/10">
        <div className="flex items-center gap-2 border-b border-border bg-background-subtle px-4 py-3 dark:bg-[#0a1012]">
        ...
        <div
          ref={contentRef}
          className="no-visible-scrollbar h-64 overflow-y-auto bg-background p-4 dark:bg-[#111a1d] sm:h-72"
        >
          {lines.map((line, i) => (
            <div key={i} className="leading-relaxed whitespace-pre-wrap">
              {line.type === "command" ? (
                ...
                <span className="text-foreground-muted dark:text-neutral-400">
                  {line.content}
                </span>
              )}
            </div>
          ...
          {phase === "typing" && (
            <div className="leading-relaxed whitespace-pre-wrap">
              ...
              <span className="ml-0.5 inline-block h-4 w-2 bg-accent align-middle dark:bg-accent-300" />
            </div>
          )}
          ...
          {(phase === "done" ||
            ...
            <div className="leading-relaxed whitespace-pre-wrap">
              ...
              <span
                className={cn(
                  "inline-block h-4 w-2 bg-accent align-middle transition-opacity duration-100 dark:bg-accent-300",
